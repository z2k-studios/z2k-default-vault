...
def test_001_001_project_structure(result: FeatureResult) -> None:
...
def test_001_002_python_version(result: FeatureResult) -> None:
...
def test_001_003_run_all_importable(result: FeatureResult) -> None:
    ...
    if proc.returncode in (0, 1) and "Traceback" not in proc.stderr:
        ...
        print_fail(req_id, desc, "quantitative",
                   ...
                   actual=proc.stderr[:500] if proc.stderr else f"exit code {proc.returncode}")
        result.failed += 1


def test_001_006_command_queue_folder(result: FeatureResult) -> None:
    ...
    if not COMMAND_QUEUE_DIR.exists():
        print_fail(req_id, desc, "quantitative",
                   ...
                   actual="Directory not found — ensure Obsidian is running with Command Queues enabled")
        result.failed += 1
        return
...
def test_001_004_005_command_queue_smoke(result: FeatureResult) -> None:
    ...
    try:
        ...
        print_fail(req_id_obs, desc_obs, "quantitative",
                   ...
                   actual="Cannot determine — packet submission failed")
        result.failed += 1
        return
    ...
    if not found:
        print_fail(req_id_cq, desc_cq, "quantitative",
                   ...
                   actual=f"No output after {COMMAND_QUEUE_TIMEOUT_SECONDS}s — Obsidian may not be running or Command Queues may be disabled")
        result.failed += 1
        print_fail(req_id_obs, desc_obs, "quantitative",
                   expected="Obsidian processes Command Queue packet",
                   actual="Timeout — Obsidian did not process the packet")
        result.failed += 1
        return
    ...
    if "Hello World!" not in content:
        print_fail(req_id_cq, desc_cq, "quantitative",
                   ...
                   actual=f"Content: {content[:200]!r}")
        result.failed += 1
    else:
    ...
    _error_log_result = new_entries  # saved for 001-007
...
def test_001_007_error_log_monitoring(result: FeatureResult, error_log_entries: list) -> None:
    ...
    if error_log_entries is None:
        ...
        print_fail(req_id, desc, "quantitative",
                   ...
                   actual="Cannot determine — Command Queue test did not complete")
        result.failed += 1
        return
    ...
    if error_log_entries:
        print_fail(req_id, desc, "quantitative",
                   ...
                   actual=f"{len(error_log_entries)} new entry/entries detected",
...
def main() -> int:
    ...
    test_001_001_project_structure(result)
    test_001_002_python_version(result)
    test_001_003_run_all_importable(result)
    test_001_006_command_queue_folder(result)

    # Smoke test returns error log entries for 001-007
    error_log_entries = test_001_004_005_command_queue_smoke(result)

    test_001_007_error_log_monitoring(result, error_log_entries)
